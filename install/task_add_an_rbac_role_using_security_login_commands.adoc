...
NOTE: Aus Konsistenzgründen bezieht sich die SnapCenter-Dokumentation auf die Rollen als Verwenden von Berechtigungen. Die OnCommand System Manager GUI verwendet den Begriff „`attribut`“ anstatt „`` Privilege“. Beim Einrichten von ONTAP RBAC-Rollen bedeuten beide Begriffe dasselbe.
